...
class StandupGenerator:
    ...
    def _is_meaningful_blocker_reason(self, reason: Optional[str], blocked_by: Optional[str]) -> bool:
        ...
        if blocked_by_l:
            # If the "reason" is just restating the blocked_by, it's not useful to print as a reason.
            if reason_l == f"blocked by {blocked_by_l}":
                return False

        # "Blocked by X" without any extra context is effectively missing.
    ...
    def _todo_sort_key(self, task: Dict[str, Any]) -> tuple:
