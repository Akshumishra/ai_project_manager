...
def create_document_with_blocks(
    ...
    blocks_data: list[schemas.BlockCreate],
    db: Session,
    current_user: User,
):
    """
    Creates a document and populates it with multiple blocks in a single operation.
    Useful for initial saves of generated documents.
    """
    project = db.query(Project).filter(Project.id == data.project_id).first()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    for i, b_data in enumerate(blocks_data):
        # If no position_key, generate sequential ones starting from 1000
        pos_key = str(1000 * (i + 1))
        block = DocumentBlock(
            doc_id=document.id,
    ...
    redis_key = f"doc:{document.id}"
    redis_client.set(redis_key, json.dumps(response), ex=3000)

    return {"document_id": str(document.id), "blocks": result_blocks}
