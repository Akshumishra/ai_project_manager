...
@router.post("/standup/finalize-all")
async def finalize_all_standups():
    """
    Finalizes all active standups from the last 24 hours and posts summaries to Slack.
    Useful for manual triggering without waiting for the scheduler.
    """
    db = SessionStandup()
    try:
        manager = StandupManager(db)
        ...
        logger.error(f"Error finalizing all standups: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))
    finally:
        db.close()
